...
#   This script only works for Relion 3.0        #
##################################################
...
#With this script, newest files cannot be linked to the folder...
import_section = IMPORT_SECTION.replace("#IMPORT_CMD1", """{add_header_cmd}""").replace("#IMPORT_CMD2", """{import_cmd}""")
...
x3 = """mpirun -np {mpi} `which relion_run_motioncorr_mpi` --i Import/job001/movies.star --o MotionCorr/job002/ --first_frame_sum 1 --last_frame_sum -1 --use_motioncor2  --motioncor2_exe {motioncorr2} --other_motioncor2_args " -Tor 0.5 -Iter 7 " --bin_factor 2 --bfactor 250 --angpix {half_apix} --voltage {voltage} --dose_per_frame {dose} --preexposure 0 --patch_x 7 --patch_y 5 --gainref {gain} --gain_rot 1 --gain_flip 0 --dose_weighting  --only_do_unfinished"""
x4 = """mpirun -np {mpi} `which relion_run_ctffind_mpi` --i MotionCorr/job002/corrected_micrographs.star --o CtfFind/job003/ --CS {cs} --HT {voltage} --AmpCnst 0.1 --XMAG 10000 --DStep 1.05 --Box 512 --ResMin 30 --ResMax 5 --dFMin 5000 --dFMax 50000 --FStep 500 --dAst 100 --use_gctf --gctf_exe {gctf} --angpix {apix} --EPA --gpu "" --only_do_unfinished"""
x5a = """mpirun -np {mpi} `which relion_autopick_mpi` --i CtfFind/job003/micrographs_ctf.star --odir AutoPick/job004/ --pickname autopick --ref {ref} --invert  --ctf  --ang 5 --shrink 0.5 --lowpass 20 --highpass 400 --angpix {apix} --angpix_ref {ref_apix} --threshold 0.2 --min_distance 90 --max_stddev_noise 1.1 --gpu "" --only_do_unfinished"""
x5b = """mpirun -np 8 `which relion_autopick_mpi` --i CtfFind/job003/micrographs_ctf.star --odir AutoPick/job004/ --pickname autopick --LoG  --LoG_diam_min {mind} --LoG_diam_max {maxd} --shrink 0 --lowpass 20 --LoG_adjust_threshold 1 --only_do_unfinished"""
x5x = """echo CtfFind/job003/micrographs_ctf.star > AutoPick/job004/coords_suffix_autopick.star"""
x6 = """mpirun -np 2 `which relion_preprocess_mpi` --i CtfFind/job003/micrographs_ctf.star --coord_dir AutoPick/job004/ --coord_suffix _autopick.star --part_star Extract/job005/particles.star --part_dir Extract/job005/ --extract --extract_size {boxsize} --scale {half_boxsize} --norm --bg_radius {bg_radius} --white_dust -1 --black_dust -1 --invert_contrast  --only_extract_unfinished"""
import_section = IMPORT_SECTION.replace("#LINK_CMD", x0).replace("#IMPORT_CMD1", x1).replace("#IMPORT_CMD2", x2)
...
y4a = """mpirun -np {mpi} `which relion_autopick_mpi` --i CtfFind/job002/micrographs_ctf.star --odir AutoPick/job003/ --pickname autopick --ref {ref} --invert  --ctf  --ang 5 --shrink 0.5 --lowpass 20 --highpass 400 --angpix {apix} --angpix_ref {ref_apix} --threshold 0.2 --min_distance 90 --max_stddev_noise 1.1 --gpu "" --only_do_unfinished"""
y4b = """mpirun -np 8 `which relion_autopick_mpi` --i CtfFind/job002/micrographs_ctf.star --odir AutoPick/job003/ --pickname autopick --LoG  --LoG_diam_min {mind} --LoG_diam_max {maxd} --shrink 0 --lowpass 20 --LoG_adjust_threshold 1 --only_do_unfinished""" 
y4y = """echo CtfFind/job002/micrographs_ctf.star > AutoPick/job003/coords_suffix_autopick.star"""
y5 = """mpirun -np 2 `which relion_preprocess_mpi` --i CtfFind/job002/micrographs_ctf.star --coord_dir AutoPick/job003/ --coord_suffix _autopick.star --part_star Extract/job004/particles.star --part_dir Extract/job004/ --extract --extract_size {boxsize} --scale {half_boxsize} --norm --bg_radius {bg_radius} --white_dust -1 --black_dust -1 --invert_contrast  --only_extract_unfinished"""
import_section = IMPORT_SECTION.replace("#LINK_CMD", y0).replace("#IMPORT_CMD1", y1).replace("#IMPORT_CMD2", y2)
...
relion_gui_parameters["manpick"] = """job_type == 3
is_continue == false
Pixel size (A) == -1
Black value: == 0
Blue value:  == 0
